...
public class GameManager : MonoBehaviour {
	...
	void OnDestroy()
	{
		Ball.manager.onBallHasBeenHitByPlayerRacket-=BallHasBeenHitByPlayerRacket;
		Ball.manager.onBallHasBeenMissedByPlayerRacket-=BallHasBeenMissedByPlayerRacket;
	...
	IEnumerator Start()
	{
        ...
        while (!(LANG.manager.IsReady
              ...
              && HudManager.manager.IsReady
              && MenuManager.manager.IsReady
              && Ball.manager.IsReady
              && EnemyRacket.manager.IsReady
              && PlayerRacket.manager.IsReady))
        {
            //print("LANG.manager.IsReady: " + LANG.manager.IsReady);
            //print("PlayersManager.manager.IsReady: " + PlayersManager.manager.IsReady);
            //print("HudManager.manager.IsReady: " + HudManager.manager.IsReady);
            //print("MenuManager.manager.IsReady: " + MenuManager.manager.IsReady);
            //print("Ball.manager.IsReady: " + Ball.manager.IsReady);
            //print("EnemyRacket.manager.IsReady: " + EnemyRacket.manager.IsReady);
            //print("PlayerRacket.manager.IsReady: " + PlayerRacket.manager.IsReady);
            yield return null;
        }
	...
	#region Ball Callbacks 
	void BallHasBeenHitByPlayerRacket()
	{
		PlayersManager.manager.IncreaseScore(1);

		if(onGameScoreHasChanged!=null)
			onGameScoreHasChanged();
	}

	void BallHasBeenMissedByPlayerRacket()
	{
